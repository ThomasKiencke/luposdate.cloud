...
public class Old_PigIndexScanOperator implements IPigOperator {
	ArrayList<JoinInformation> intermediateJoins = null;
	Collection<TriplePattern> triplePatternCollection = null;
	int tripleCounter = 0;
	boolean debug = false;

	public Old_PigIndexScanOperator(Collection<TriplePattern> tp) {
		...
		for (TriplePattern triplePattern : this.triplePatternCollection) {
			...
			if (curPattern.allElementsAreVariables()) {
				...
				result.append(curPattern.getName()
						+ " = foreach "
						+ curPattern.getTablename()
						+ "_DATA generate $0, flatten(lupos.cloud.pig.udfs.ListToBagUDF($1));\n");
				...
				result.append(") as (columncontent_" + tripleCounter
						+ ":bytearray);\n");

				result.append(curPattern.getName()
						+ " = foreach PATTERN_"
						+ curPattern.getPatternId()
						+ " generate flatten(lupos.cloud.pig.udfs.ListToBagUDF("
						+ "$0 )) as "
